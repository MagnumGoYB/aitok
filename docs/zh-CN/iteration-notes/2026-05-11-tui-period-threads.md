...
- 复制选中 thread 时只复制 thread/session ID，使用 OSC52。对支持的 thread 按 Enter 会启动本地工具的 resume 命令，不改变复制契约。
...
- 旧的 `t threads` 聚焦快捷键已从用户可见帮助中移除；`j/k/home/end/enter/c` 直接作用于过滤后的 Threads 列表。
...
- 后续可观察 Gemini CLI 是否提供稳定的本地 resume 命令，再接入 TUI Enter 动作。
